...
demographiedaten_total <- read_delim("data/EWR202012E_Matrix.csv", delim = ";", show_col_types = FALSE) %>% 
  filter(BEZ == "12") %>% 
  summarise('total' = sum(E_E)) %>% 
  pull()
...
resultslist$incidence_total <- df %>% 
    ...
    summarise(n = n(), .groups = 'drop') %>% 
    mutate(totalpopulation = demographiedaten_total) %>% 
