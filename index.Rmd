...
# Read in data
externalinput <- tar_read(externalinput) # Here we read in self written data 
demographiedata <- tar_read(demographiedata) # Data about the population
df <- tar_read(df) # SurvNet data

## Filtering
methodslist <- tar_read(store_info_about_filtering)
df <- tar_read(df_filtered)
...
library(targets)
list_deduplicated <- tar_read(list_deduplicated)

  "data/results.csv" %>%
    read_csv(col_types = cols(.default = "c")) %>% 
    bind_rows(c(varname = "doppeltefaelle", "value" = nrow(doppelte_faelle_df) - nrow(doppelte_faelle_df_bereinigt))) %>% 
    bind_rows(c(varname = "doppeltekps", "value" = nrow(doppelte_kps_df) - nrow(doppelte_kps_df_bereinigt))) %>% 
